...
def start_scan_for_target(target):
    ...
    with _lock:
        ...
        if len(_active_scans) >= 3:
            logger.warning(f"Global active scan limit (3) reached. Cannot start scan for target: {url}")
            return False
...
def _should_run_step(step_name, resume_status):
    if not resume_status: return True
    steps = [
        ...
        "Running Shodan", "Running Wayback Machine", "Running CRT.sh",
        "Running HackerTarget", "Running Whois", "Correlating CVEs", "Report Pending"
    ]
    if resume_status not in steps: return True
    if step_name not in steps: return True
    return steps.index(step_name) >= steps.index(resume_status)

def _run_scan_sequence(target, resume_scan_id=None, resume_status=None):
    target_id = target["id"]
    url = target["url"]
...
def resume_interrupted_scans():
    """Find scans that were running and resume them."""
    try:
        conn = get_db_connection()
        # Find scans not Completed and not Failed
        rows = conn.execute(
            "SELECT scans.*, targets.url FROM scans "
            "JOIN targets ON scans.target_id = targets.id "
            "WHERE scans.status != 'Completed' AND scans.status != 'Failed' AND scans.status != 'Report Pending'"
        ).fetchall()
        conn.close()

        def _get_resume_status_from_log(target_url, db_status):
            import os
            ...
            step_map = {
                "[1/": "Running Traceroute", "[2/": "Running HTTPx", "[3/": "Running WhatWeb",
                "[4/": "Running Subfinder", "[5/": "Running Nmap", "[6/": "Running SSL Scan",
                "[7/": "Running Nikto", "[8/": "Running Nuclei", "[9/": "Running ffuf",
                "[10/": "Running ZAP", "[11/": "Running Wapiti", "[12/": "Running SQLMap"
            }
            last_status = None
            try:
                ...
                pass
            return last_status if last_status else db_status
        ...
        for r in rows:
            ...
            with _lock:
                if target_id in _active_scans:
                    continue
                if len(_active_scans) >= 3:
                    logger.warning(f"Global active scan limit (3) reached. Cannot resume scan for {target['url']}")
                    continue
                
                resume_phase = _get_resume_status_from_log(target["url"], scan["status"])
                
                thread = threading.Thread(
                    target=_run_scan_sequence,
                    args=(target, scan["id"], resume_phase),
                    daemon=True,
                    name=f"ScanThread_{target_id}",
                )
                _active_scans[target_id] = thread
                thread.start()
                logger.info(f"Resumed scan {scan['id']} for {target['url']} from {resume_phase}")
    except Exception as e:
