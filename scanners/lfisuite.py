...
def scan(target_url: str, scan_id: int, settings: dict):
    ...
    if get_setting("scan_profile", "standard") not in ["full", "full"] and "full" == "full":
        logger.info(f"[lfisuite] Skipping — requires 'full' profile")
        return []
    ...
    try:
        ...
        logger.warning(f"[lfisuite] Binary not found, skipping.")
        return None
    except subprocess.TimeoutExpired:
        logger.warning(f"[lfisuite] Timed out.")
        return []
    ...
    if not findings and result.stdout:
        logger.debug(f"[lfisuite] Output recorded but no direct vulns parsed.")
