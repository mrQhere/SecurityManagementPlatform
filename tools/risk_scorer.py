"""
Risk Scoring Engine V9.3.2 — calibrated against real CVE data.
...
def calculate_and_store_risk_score(scan_id, findings):
    ...
    V9.3.2: Reads cvss_score and epss_score columns directly — no regex parsing.
    ...
    for f in findings:
        ...
        # V9.3.2: Read CVSS/EPSS directly from the findings row columns
        ...
        if tool == "CVE Correlation":
            # V9.3.2: Use CVSS column directly (no regex needed)
    ...
    logger.info(
        f"Risk Score V9.3.2: {score}/100 ({rating}) — "
