"""
Risk Scoring Engine V7.0.6 — calibrated against real CVE data.
...
def calculate_and_store_risk_score(scan_id, findings):
    ...
    V7.0.6: Reads cvss_score and epss_score columns directly — no regex parsing.
    ...
    for f in findings:
        ...
        # V7.0.6: Read CVSS/EPSS directly from the findings row columns
        ...
        if tool == "CVE Correlation":
            # V7.0.6: Use CVSS column directly (no regex needed)
    ...
    logger.info(
        f"Risk Score V7.0.6: {score}/100 ({rating}) — "
