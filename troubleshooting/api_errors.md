...
```bash
# Re-authenticate
TOKEN=$(curl -s -X POST http://localhost:8000/api/v9.4.3/auth/token \
...
Request body is malformed. Check the schema at `http://localhost:8000/api/v9.4.3/docs`.
...
```bash
curl http://localhost:8000/api/v9.4.3/scan/1/status \
